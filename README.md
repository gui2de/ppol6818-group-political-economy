...
- Research Design
Given the nature of newspaper closures as a natural experiment, we will use a Difference-in-Differences (DiD) analysis to compare communities that lost a local newspaper (treatment group) with those that retained their newspaper (control group). This method will help isolate the causal effect of newspaper closures on political polarization by comparing pre- and post-closure trends while controlling for initial differences.
...
2. Define Treatment and Control Groups  
  (1) Treatment Group: Counties that used to have one or more local newspapers but lost them.
  (2) Control Group: Counties that retained at least one local newspaper.
